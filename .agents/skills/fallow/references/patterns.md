...
- name: Dead code check
  run: bunx fallow dead-code --fail-on-issues --quiet
```
...
- name: Fallow analysis
  run: bunx fallow dead-code --ci > fallow.sarif
...
- name: Check for new dead code
  run: bunx fallow dead-code --format json --quiet --changed-since ${{ github.event.pull_request.base.sha }} --fail-on-issues
```
...
- name: Duplication check
  run: bunx fallow dupes --format json --quiet --threshold 5 --mode mild
```
...
- name: Check duplication in changed files
  run: bunx fallow dupes --format json --quiet --changed-since ${{ github.event.pull_request.base.sha }}
```
...
fallow:
  image: node:20-slim
  script:
    - bunx fallow dead-code --fail-on-issues --quiet --format json > fallow-results.json
  artifacts:
    paths:
      - fallow-results.json
```
...
- Resolves `fallow` from PATH first, then `bunx --no-install fallow` as a fallback. Skips with a stderr notice if neither is available or if `jq` is missing.
...
Codex fallback (add to `.agents/instructions/build.md`):
...
Removes the fallow-gate handler from `.claude/settings.json` (preserving any other handlers in the same matcher group), deletes `.claude/hooks/fallow-gate.sh` if it still carries the `# Generated by fallow setup-hooks.` marker, and strips the managed block from `.agents/instructions/build.md`. Idempotent: a second run reports `unchanged` / `not present` and exits 0.
